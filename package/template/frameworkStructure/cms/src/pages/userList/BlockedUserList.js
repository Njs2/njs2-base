...
import UnblockUserModal from '../profile/UnblockUserModal';
...
export default function BlockedUserList(props) {
  ...
  const [unblockUserModalShow, setUnblockUserModalShow] = useState(false);
  const [unblockUserId, setUnblockUserId] = useState();
  const [filterType, setFilterType] = useState(0);

  const loadUsersList = async (reset, force = false) => {
    if (!force)
      if ((reset && inProgress) || !loadMoreData) return;
    setInProgress(true);
    const res = await apiClient('/admin/search_users', 'GET', null, { type: 2, last_index: reset ? 0 : lastIndex, search_text: searchText, filtered_on: filterType }, props.accessToken);
    ...
    <Container className="" style={containerStyle} onScroll={scrollCheck} onLoad={heightCheck}>
      ...
      <div>
          ...
          {userList.length !== 0 && <UserTable
            ...
            usersData={userList.map(user => {
              ...
              return {
                ...
                sign_up_date: user.blocked_timings ? new Date(user.blocked_timings.start_time).toLocaleDateString() : "",
                btnText: "Unblock",
        ...
        <UnblockUserModal
          ...
          isOpen={unblockUserModalShow}
          toggle={() => setUnblockUserModalShow(false)}
          loadUsersList={() => { setLoadMoreData(true); loadUsersList(true, true); }}
        />
      </div>
    </Container>
  );
